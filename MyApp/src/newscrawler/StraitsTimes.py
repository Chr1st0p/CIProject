...
from MyApp.src.parsers.DateParser import StraitsTimesDateParse
import re
...
def getHtml(iters):
    ...
    html = requests.get(url=targeturl, headers=RequestHeader.browserHeader, proxies=proxies)
...
def parseContentKeyword(newsurl):
    html = requests.get(url=newsurl, headers=RequestHeader.browserHeader,proxies=proxies)
    category = ''
    date = ''
    try:
